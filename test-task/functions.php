...
function test_task_setup() {
	/*
		...
		* If you're building a theme based on test-task, use a find and replace
		* to change 'test-task' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'test-task', get_template_directory() . '/languages' );
...
function test_task_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'test-task' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'test-task' ),
...
function test_task_scripts() {
	...
	wp_style_add_data( 'test-task-style', 'rtl', 'replace' );

	wp_enqueue_script( 'test-task-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );


	wp_register_script( 'ajax-script', get_template_directory_uri() . '/js/ajax-loadmore.js', array( 'jquery' ), time() );
